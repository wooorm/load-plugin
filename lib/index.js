...
/** @type {Promise<URL> | URL | undefined} */
let globalFolder
...
export async function resolvePlugin(name, options) {
  ...
  if (typeof settings.global === 'boolean') {
    ...
    globals = argv.startsWith(npmPrefix)
  }
  ...
  if (name.charAt(0) !== '.') {
    if (globals) {
      if (!globalFolder) globalFolder = inferGlobalFolder()
      if (typeof globalFolder !== 'string') globalFolder = await globalFolder
 ...
 * @returns {Promise<URL>}
 */
async function inferGlobalFolder() {
  /* c8 ignore next 3 */
  if (!npmPrefix) npmPrefix = inferNpmPrefix()
  if (typeof npmPrefix !== 'string') npmPrefix = await npmPrefix
  ...
  let folder = new URL(nodeModules, pathToFileURL(npmPrefix) + '/')
  ...
  if (electron && nvm) {
    try {
      ...
      folder = new URL(nodeModules, pathToFileURL(nvm))
    } catch {
      // Doesn’t exist.
    }
  }
